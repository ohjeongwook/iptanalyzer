...
def decode_blocks_function(pt_filename, dump_filename, queue, temp_foldername, log_directory):
    ...
    while True:
        ...
        try:
            ...
            logging.debug("# decode_blocks_function DecodeBlocks Exception: %s" % tb)
...
if __name__ == '__main__':
    ...
    for i in range(0, process_count, 1):
        proc = multiprocessing.Process(target = decode_blocks_function, args=(args.pt_filename, args.dump_file, pqueue, args.temp, args.log_directory))
        procs.append(proc)
        proc.start()
