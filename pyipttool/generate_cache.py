...
def decode_block(pt_filename, dump_filename, temp_directory, cache_filename, start_offset = 0, end_offset = 0):
    logging.debug("decode_block: dump_filename: %s, cache_filename: %s" % (dump_filename, cache_filename))
    pt_log_analyzer = pyipttool.ipt.Analyzer(dump_filename, dump_symbols = False, load_image = True, temp_directory = temp_directory)
    ...
    try:
        ...
        logging.debug("# decode_blocks_function exception: %s" % str(tb))

    logging.debug("# decode_blocks_function: Writing %.16x ~ %.16x to %s" % (start_offset, end_offset, cache_filename))
    if cache_filename:
        try:
            ...
            tb = traceback.format_exc()
            logging.debug("# decode_blocks_function WriteBlockOffsets Exception: %s" % tb)

def decode_blocks_function(pt_filename, dump_filename, queue, temp_directory, log_directory):
    ...
    while True:
        ...
        decode_block(pt_filename, dump_filename, temp_directory, cache_filename, start_offset, end_offset)
...
if __name__ == '__main__':
    ...
    if not use_multiprocess:
        logging.basicConfig(
            ...
            handlers = [
                #logging.FileHandler(os.path.join(args.log_directory, str(uuid.uuid1()) + '.log')),
                logging.StreamHandler()
        ])
        decode_block(args.pt_filename, args.dump_filename, args.temp_directory, args.cache_filename)
    else:
        import multiprocessing

        ipt_analyzer = pyipttool.ipt.Analyzer(args.dump_filename, dump_symbols = False, load_image = False, progress_report_interval = 100)
        ...
        for i in range(0, process_count, 1):
            proc = multiprocessing.Process(target = decode_blocks_function, args=(args.pt_filename, args.dump_filename, pqueue, args.temp_directory, args.log_directory))
            procs.append(proc)
            proc.start()
